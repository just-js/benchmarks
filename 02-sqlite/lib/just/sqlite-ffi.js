...
if (!handle) throw new Error('Could not create handle')
...
function wrapOpen () {
  ...
  return fileName => {
    ...
    const r = dv.call()
    if (r !== codes.SQLITE_OK) throw new SQLiteError('sqlite3_open', r)
    return fp2.getBigUint64(0, true)
  }
}

function wrapOpenV2 () {
  const dv = createCif('sqlite3_open_v2', [FFI_TYPE_POINTER, FFI_TYPE_POINTER, FFI_TYPE_SINT32, FFI_TYPE_POINTER])
  // const char* filename
  const fp = new DataView(new ArrayBuffer(8))
  dv.setBigUint64(0, fp.buffer.getAddress(), true)
  // sqlite3** db
  ...
  // const char* vfs
  const fp4 = new DataView(new ArrayBuffer(8))
  dv.setBigUint64(24, fp4.buffer.getAddress(), true) 
  return (fileName, flags) => {
    const buf = ArrayBuffer.fromString(`${fileName}\0`)
    fp.setBigUint64(0, buf.getAddress(), true)
    av.setInt32(0, flags, true)
    const r = dv.call()
    if (r !== codes.SQLITE_OK) throw new SQLiteError('sqlite3_open_v2', r)
    return fp2.getBigUint64(0, true)
  }
}
...
const prepare = wrapPrepare()

function prepareStatement (db, sql) {
  const { stmt } = prepare(db, sql)
  const { SQLITE_ROW } = codes

  const columnIntCif = createCif('sqlite3_column_int', [FFI_TYPE_POINTER, FFI_TYPE_UINT32], FFI_TYPE_UINT32)
  const fp = new DataView(new ArrayBuffer(8))
  columnIntCif.setBigUint64(0, fp.buffer.getAddress(), true)
  const av = new DataView(new ArrayBuffer(4))
  columnIntCif.setBigUint64(8, av.buffer.getAddress(), true)

  const stepCif = createCif('sqlite3_step', [FFI_TYPE_POINTER], FFI_TYPE_UINT32)
  stepCif.setBigUint64(0, fp.buffer.getAddress(), true)
  const resetCif = createCif('sqlite3_reset', [FFI_TYPE_POINTER], FFI_TYPE_UINT32)
  resetCif.setBigUint64(0, fp.buffer.getAddress(), true)
  const finalizeCif = createCif('sqlite3_finalize', [FFI_TYPE_POINTER], FFI_TYPE_UINT32)
  finalizeCif.setBigUint64(0, fp.buffer.getAddress(), true)

  fp.setBigUint64(0, stmt, true)
  av.setInt32(0, 0, true)

  return () => {
    if (stepCif.call() === SQLITE_ROW) {
      const value = columnIntCif.call()
      resetCif.call()
      return value
    }
    finalizeCif.call()
