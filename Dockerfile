FROM ubuntu:latest AS js-benchmark-builder
RUN apt update
RUN apt install -y curl unzip xz-utils libsqlite3-dev gzip make tar g++
WORKDIR /bench
...
RUN export PATH=$PATH:/root/.bun/bin/
WORKDIR /root/.just
...
RUN export PATH=$PATH:/root/.just/bin/
WORKDIR /bench/node
